...
{
	Load("../../../../../../../../Documents/Repository/BitPaddle!/BitPaddle!/images/ball.png");
	assert(IsLoaded());
...
void GameBall::Update(float elapsedTime) //Parameter is the time since last frame in seconds. VERY small number.
{
	...
	else
	{
		...
		PlayerPaddle* player1 = dynamic_cast<PlayerPaddle*>(Game::GetGameObjectManger().Get("Paddle1"));
		if(player1 != NULL)
		{
			...
			if(p1BB.intersects(GetBoundingRect()))       //(GetPosition().x + moveByX + (GetSprite().GetSize().x /2),GetPosition().y + (GetSprite().GetSize().y /2) + moveByY))
			{
				...
				ServiceLocator::GetAudio()->PlaySound("/Users/Anarchy/Documents/Repository/BitPaddle!/BitPaddle!/kaboom.wav");
				_velocity += 5.0f;
			}
